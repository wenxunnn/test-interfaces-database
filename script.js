...
function renderPoscarPreview(system) {
  ...
  viewerElement.classList.remove("has-3dmol");

  const poscar = system.files?.poscar || buildCsvReferencePoscar(system.raw || {});
  const atoms = parsePoscarAtoms(poscar);

  if (window.$3Dmol && atoms.length > 0) {
    render3DmolViewer(viewerElement, atoms);
    document.getElementById("viewerAtomCount").textContent = `${atoms.length} atoms rendered from POSCAR`;
    return;
  }

  renderFallbackAtomPreview(viewerElement, system);
}
...
function render3DmolViewer(viewerElement, atoms) {
...
function resetViewerCamera() {
  if (!crystalViewer) return;
  crystalViewer.zoomTo();
  crystalViewer.rotate(0);
  crystalViewer.render();
}

function resizeCrystalViewer() {
  if (!crystalViewer) return;
  window.setTimeout(() => {
...
function renderFallbackAtomPreview(viewer, system) {
  ...
  note.textContent = "3Dmol.js is unavailable, so this fallback POSCAR preview is shown.";
