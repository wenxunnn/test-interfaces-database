...
const EFORM_JSON_URL = "eform_vs_ratio_data.json";
...
async function loadEformGraphData() {
  const status = document.getElementById("eformGraphStatus");
  try {
    const response = await fetch(EFORM_JSON_URL, { cache: "no-store" });
    if (!response.ok) throw new Error(`Could not load ${EFORM_JSON_URL}`);
    eformGraphDataset = await response.json();
    if (status) status.textContent = "";
  } catch (error) {
    console.error(error);
    eformGraphDataset = null;
    if (status) status.textContent = `Graph data not loaded. Make sure ${EFORM_JSON_URL} is in the same folder as index.html.`;
  }
  renderEformGraph();
}
...
function renderResults(systems, unfilteredCount = systems.length) {
  ...
  if (systems.length === 0) {
    ...
    const helper = unfilteredCount === 0
      ? "This explorer has no rows in the current CSV/data source yet."
