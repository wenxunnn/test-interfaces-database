...
function searchSingleExplorer(type) {
  ...
  currentMatches = explorerPool.filter(system => {
    ...
    return hasExactElementSet(selected, elements);
  });
  ...
  currentActiveFilterHtml = selected.length === 0
    ...
    : `<span>${labelMap[type]} exact elements: <strong class="mono">${selected.join(", ")}</strong></span>`;
...
function openSystemFromGraphPoint(point) {
  ...
  const matches = databaseSystems.filter(system => {
    const systemKey = normalizeSurfaceKey(system.name);
    return aliases.some(alias => alias && (systemKey.includes(alias) || alias.includes(systemKey)));
  });
