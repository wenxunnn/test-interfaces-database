...
function searchSingleExplorer(type) {
  ...
  currentActiveFilterHtml = selected.length === 0
    ? `<span>Showing all ${labelMap[type].toLowerCase()} entries &mdash; no element filters applied.</span>`
...
function getSystemElementsForExplorer(system, type) {
  ...
  const text = [system.type, system.materialA, system.materialB, system.formula, system.system, system.name, system.sourceSheet].join(" ");
...
function updateStats() {
  ...
  const overlayers = new Set(databaseSystems.flatMap(system => system.overlayerElements));
  const bulkElements = new Set(databaseSystems.flatMap(system => getSystemElementsForExplorer(system, "bulkExplorer")));
  ...
  setText("statOverlayerSystems", databaseSystems.filter(system => system.overlayerElements.length).length);
  setText("statOverlayerElements", overlayers.size);
  setText("statSubstrateSystems", databaseSystems.filter(system => system.substrateElements.length).length);
  setText("statSubstrateElements", substrates.size);
  setText("statBulkSystems", databaseSystems.length);
...
buildPeriodicTable("overlayerExplorerTable", "overlayerExplorer");
buildPeriodicTable("substrateExplorerTable", "substrateExplorer");
buildPeriodicTable("bulkExplorerTable", "bulkExplorer");
